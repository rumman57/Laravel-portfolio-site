...
@section('content')
  ...
  <div class="content-wrapper">
    ...
    <section class="content-header">
      <h1>
        Show Portfolio Services
      </h1>
      <ol class="breadcrumb">
        <li><a href="#"><i class="fa fa-dashboard"></i> Home</a></li>
        <li><a href="#">Portfolio Services</a></li>
        <li class="active">Show Portfolio Services</li>
      </ol>
    </section>
    <!-- Main content -->
            ...
            <div class="box-body">
              ...
              <?php $i++;?>
                <tr>
                  <td>{{$i}}</td>
                  <td>{{$service->name}}</td>
                  <td>{{$service->data_type}}</td>
                  <td>{{$service->priority}}</td>
                  <td>
                     <a href="{{route('portfolo-services.edit',$service->id)}}"><button class="btn btn-primary">Edit</button></a>
                  </td>
                  <td>
                    <form method="POST" action="{{action('PortfolioServiceController@destroy',['id'=>$service->id])}}">
                        {{method_field('DELETE')}}
                        {{csrf_field()}}
