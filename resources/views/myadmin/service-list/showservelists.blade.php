            ...
            <div class="box-body">
              ...
              <?php $i++;?>
                <tr>
                  <td>{{$i}}</td>
                  <td>{{$servlist->name}}</td>
                  <td>{{$servlist->service->name}}</td>
                  <td>
                     <a href="{{route('service-list.edit',$servlist->id)}}"><button class="btn btn-primary">Edit</button></a>
                  </td>
                  <td>
                    <form method="POST" action="{{action('ServiceListController@destroy',['id'=>$servlist->id])}}">
                        {{method_field('DELETE')}}
                        {{csrf_field()}}
